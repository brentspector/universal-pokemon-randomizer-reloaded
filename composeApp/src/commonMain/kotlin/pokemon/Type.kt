...
enum class Type(isHackOnly: Boolean = false) {
    ...
    fun randomStrength(random: RandomSource, useResistantType: Boolean, vararg checkTypes: Type?): Type? {
    ...
    fun getStrengthFromList(random: RandomSource, checkMap: Map<Type, List<Type>>, vararg checkTypes: Type?): Type? {
    ...
    fun randomWeakness(random: RandomSource, useResistantType: Boolean, vararg checkTypes: Type?): Type? {
        // Safety check since varargs allow zero arguments
        if (checkTypes.isEmpty()) {
    ...
    private fun getWeaknessFromList(random: RandomSource, checkMap: Map<Type, List<Type>>, vararg checkTypes: Type?): Type? {
        val pickList = checkTypes
            ...
            .fold(null as Set<Type>?) { acc, list -> acc?.intersect(list.toSet()) ?: list.toSet() }
